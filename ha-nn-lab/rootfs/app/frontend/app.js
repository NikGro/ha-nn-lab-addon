...
async function refreshProjects(){
  ...
  projects=d.projects||[];
  if(current && !find(current)) current=null;
  renderProjectList();
...
function renderProjectList(){
  ...
  projects.forEach(x=>{
    ...
    d.className='item'+(x.id===current?' active':'');
    d.innerHTML=`<b>${x.name}</b><div class='small'>${x.id}</div>`;
    d.onclick=()=>selectProject(x.id);
    p.appendChild(d)
  });
  if(!projects.length)p.innerHTML="<div class='small'>Noch keine Projekte.</div>";
}
...
let graphView={x:0,y:0,w:800,h:340};
...
async function loadGraph(){if(!current)return;const g=await api('/api/projects/'+current+'/nn-graph');renderGraphData(g)}
...
function initGraphInteraction(){
  ...
  svg.addEventListener('wheel',(e)=>{
    ...
    graphView.w=Math.max(120,Math.min(3000,graphView.w*factor));
    graphView.h=Math.max(90,Math.min(2000,graphView.h*factor));
  ...
  window.addEventListener('mousemove',(e)=>{
    if(!dragging) return;
    const dx=(e.clientX-last.x)*(graphView.w/(svg.clientWidth||1));
    const dy=(e.clientY-last.y)*(graphView.h/(svg.clientHeight||1));
    graphView.x-=dx; graphView.y-=dy; last={x:e.clientX,y:e.clientY};
    svg.setAttribute('viewBox',`${graphView.x} ${graphView.y} ${graphView.w} ${graphView.h}`);
  });
